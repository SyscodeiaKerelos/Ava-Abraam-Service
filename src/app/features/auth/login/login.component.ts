...
@Component({
  ...
  template: `
    <div class="rounded-3xl border border-slate-200/70 bg-white/70 p-8 text-slate-900 shadow-2xl backdrop-blur-2xl dark:border-white/15 dark:bg-slate-900/55 dark:text-slate-50">
      <div class="flex flex-col items-center mb-8">
        ...
        <p class="text-slate-500 dark:text-slate-400 mt-2 text-sm">{{ 'translate_auth-subtitle' | translate }}</p>
      </div>
      ...
      <form [formGroup]="loginForm" (ngSubmit)="onSubmit()" class="space-y-6">
        <div class="space-y-2">
          ...
          <div class="relative w-full">
            <i class="pi pi-envelope absolute z-10 top-1/2 -translate-y-1/2 left-3 text-slate-400"></i>
            <input 
              pInputText 
              type="email" 
